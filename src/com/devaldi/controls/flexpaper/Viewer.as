...
//				s = unescape(s);
		...
		private function textSelectorMoveHandler(event:MouseEvent):void{
			...
			//lianghj 划线取字
			var cIndex:int = snap.hitTestTextNearPos(event.target.parent.mouseX,event.target.parent.mouseY,10);
			if(cIndex == -1){
				cIndex = (_lastHitIndex==-1)?0:_lastHitIndex;
			}else{
				cIndex = cIndex +((_firstHitIndex==-1)?0:1);
			}
//			var hitIndex:int = cIndex+((_firstHitIndex==-1)?0:1);
			var hitIndex:int = cIndex
